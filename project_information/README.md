...
| [**`04_DATABASE_ARCHITECTURE_AND_BUILD_SYSTEM.md`**](04_DATABASE_ARCHITECTURE_AND_BUILD_SYSTEM.md) | The 5 database subsystems under `backend/processed_data/` (Relational SQLite, Vector Qdrant, Graph NetworkX, Pricing DPCO, Contracts SLA) and the master `build_all.py` rebuild engine. |
...
```bash
# Clean and rebuild all 5 databases (mock/local environment)
