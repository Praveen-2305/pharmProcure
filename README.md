...
- **Vector RAG (Qdrant):** Finds relevant contract clauses via high-performance semantic vector similarity (768-dimensional dense embeddings).
...
- **AI Orchestration:** LangGraph & LangChain (`src/agents/workflow.py`)
- **LLM Reasoning:** Google Gemini
- **Vector Store:** Qdrant Database (`src/rag_pipeline/vector_store.py`, `src/rag_pipeline/embedding_pipeline.py`)
- **Graph Store:** NetworkX Property Graph (`src/rag_pipeline/graph_store.py`, GraphML + JSON ontology)
- **Relational Storage:** SQLite 3 (`backend/processed_data/procurement_cases.db`) & thread-safe in-memory `CaseStore` (`src/db/session.py`)
- **Idempotent Build Engine:** Master build pipeline (`backend/build/build_all.py`) rebuilding all 5 storage layers
- **Regulatory Due Diligence:** Multi-source web scraper (`src/rag_pipeline/web_scraper.py`)
...
│   │   ├── vector/                     # Dense 768-dim vector embeddings & collection metadata
...
| 🗄️ [**`04_DATABASE_ARCHITECTURE_AND_BUILD_SYSTEM.md`**](project_information/04_DATABASE_ARCHITECTURE_AND_BUILD_SYSTEM.md) | The 5 database subsystems under `backend/processed_data/` and the master idempotent `build_all.py` engine. |
